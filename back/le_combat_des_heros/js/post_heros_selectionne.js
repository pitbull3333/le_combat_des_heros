...
router.post("/",erreur_post(async (req,res) => {
  ...
  let jeton;
  switch (true){
    case (documents[0].j1.h1.nom_heros == "" && documents[0].j2.h1.nom_heros == ""):
      jeton = 0
      break;
    case (documents[0].j1.h1.nom_heros !== "" || documents[0].j2.h1.nom_heros !== ""):
      jeton = Math.floor(Math.random() * 2) + 1;
      break;
  }
  await db.collection("collection_partie").updateOne({nom_partie:req.body.nom_partie},{$set:{
    ...
    [`j${numero_joueur}.h1.carte`]:2,
    [`j${numero_joueur}.h2.carte`]:2,
    [`j${numero_joueur}.h3.carte`]:2,
    [`j${numero_joueur}.h4.carte`]:2,
    [`j${numero_joueur}.h5.carte`]:2,
    [`j${numero_joueur}.h6.carte`]:2
  }});
